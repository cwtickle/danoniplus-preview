...
const serverData = <?php echo json_encode([
    ...
    // -----------------------------
    'version' => [
        'selected' => ($_POST['v'] ?? '') !== '' ? $_POST['v'] : ($_GET['v'] ?? ''),
        'param'    => $getParamPath ?? '',
        ...
        <tr>
            <td>
                <p style="text-align:center;">
                    <span class="title"><span class="title1">D</span>ancing☆<span class="title2">O</span>nigiri <span class="title3">P</span>review (<a href="./index2.php" onclick="return confirm('Data will be reset. Is it OK?\nデータはリセットされます。よろしいですか？');">Index</a>)</span>
                ...
                <hr>
                <form method="post" action="./index2.php" id="formV" name="formV" style="text-align:center;" enctype="multipart/form-data">
                    ...
                    <table style="width:100%;text-align:left;border:1px solid #999999;">
                        ...
                        <tr>
                            <td>HTML Template<br>( <a onclick="confirmCancelFile(`htmlf1`, `htmlf`);">Reset</a> )<br>( <a href="template2.html" download>DL Template</a> )</td>
                            <td>
                ...
                const musicData_g = `|musicTitle=musicTitle,artistName,${urlDomain}|musicUrl=(..)/tmp/nosound.mp3|`;
                ...
                if (dosData.indexOf('|musicTitle=') !== -1) {
                    document.getElementById('dos').value += `|musicUrl=(..)/tmp/nosound.mp3|`;
                } else {
                ...
                imgs.forEach((img, j) =>
                    document.getElementById('dos').value = document.getElementById('dos').value.replaceAll(img, imgf[j]));

                // ダウンロードボタンの処理
                const loadButton = document.getElementById('loadButton');

                loadButton.addEventListener('click', async () => {
                    const htmlFile = `${document.getElementById('htmlf').value}` || 'template2.html';
                    ...
                    document.getElementById(txt).addEventListener('blur', () => {
                        ...
                        if (txt === `queryParams`) {
                            if (document.getElementById(txt)?.value !== ``) {
                                document.getElementById('formV').action = `./index2.php?` + document.getElementById(txt).value;
                            } else {
                                document.getElementById('formV').action = `./index2.php`;
                            }
                        }
                    });
                });
                    ...
                    if (dosData !== null) {
                        ...
                        setTimeout(_ => {
                            if (compareVersions(baseVersion, '27.1.0') >= 0) {
                                document.getElementById('w').value = `${Math.min(Math.ceil(g_sWidth / 50) * 50, 1100)}px`;
                            }
                ...
                const getVersion = obj => {
                    ...
                    document.getElementById('formV').action = `./index2.php` + queryParams;
