...
const initDanoniPreviewClassic = () => {
    ...
    // danoni.js が baseVersion をグローバル変数として直接参照するため window に公開
    ...
    if (compareVersions(baseVersion, '0.53.x') < 0) {
        if (!hasStringMarker(dosData, '|setColor=')) {
            document.getElementById('dos').value += `|setColor=0xcccccc,0xff9999,0xffffff|`;
        }
        if (!hasStringMarker(dosData, '|frzColor=')) {
            document.getElementById('dos').value += `|frzColor=0x999999,0x999999,0x999999,0x999999,0x999999|`;
        }
    ...
    if (compareVersions(baseVersion, '0.43.x') < 0 && !hasStringMarker(dosData, '|difData=') && difData_g === '') {
        document.getElementById('dos').value += `|difData=7,Normal,3.5|`;
    }
    ...
    // HTML側のイベントハンドラ（onclick / onchange）から呼び出せるように window へ公開
